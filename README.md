...
Following are the ideas I have gathered while working with microservices at TransferWise.

## Ideas on the macro design

### Microservice size
There are 2 factors to consider when deciding how big/small should a service be: technical scalability and social scalability. 
...
If we indeed need to split things into microservices we should be careful [not to replicate the same antipatterns we might have in the current (legacy) codebase](https://medium.com/@urgo/how-to-avoid-entity-services-58bacbe3ee0b). 
Otherwise, we may just replace the [monolith with a distributed monolith](https://medium.com/transferwise-engineering/from-monolith-to-distributed-monolith-fd53d8dbbeba).
...
Often there is separate Service for each Entity which then encapsulates all access to that Entity. 
...
TODO
We believe that having vertical cross-cutting and self-contained modules is more important than focusing on separating
logic by stereotypes or horizontal layers. For example, should all repository JDBC implementations sit closer to each other
or should they be closer to the module that defines Repository interfaces and uses these. In such case we prefer putting
implementation under the module package. Just create separate subpackage under `impl`.
...
### TODO
https://medium.com/transferwise-engineering/poka-yoke-in-software-design-e6a0d955a4d8

https://medium.com/transferwise-engineering/implementing-entity-states-as-separate-classes-abc3c745fa2
...
If we do that then unit tests can be [great source for pushing our design](https://medium.com/transferwise-engineering/poka-yoke-in-software-design-e6a0d955a4d8).

If we fail to pick these benefits from unit tests it might be better to focus on writing more course grained tests. 
